...
impl TaskStore for SqliteRepository {
    async fn upsert_task(&self, task: &Task) -> Result<(), StorageError> {
        sqlx::query("INSERT INTO tasks(id,agent_id,device_id,conversation_scope_hash,title,source,project_folder,allow_execute,status,active_session_id,generation,stopped_at_ms,created_at_ms,updated_at_ms) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET agent_id=excluded.agent_id,device_id=excluded.device_id,conversation_scope_hash=excluded.conversation_scope_hash,title=excluded.title,source=excluded.source,project_folder=excluded.project_folder,allow_execute=excluded.allow_execute,status=excluded.status,active_session_id=excluded.active_session_id,generation=excluded.generation,stopped_at_ms=excluded.stopped_at_ms,updated_at_ms=excluded.updated_at_ms")
            .bind(task.id.as_str()).bind(task.agent_id.as_ref().map(AgentId::as_str)).bind(task.device_id.as_str())
            .bind(&task.conversation_scope_hash).bind(&task.title).bind(&task.source).bind(&task.project_folder).bind(task.allow_execute).bind(task.status.as_str())
    ...
    async fn task(&self, id: &TaskId) -> Result<Option<Task>, StorageError> {
        let row = sqlx::query("SELECT id,agent_id,device_id,conversation_scope_hash,title,source,project_folder,allow_execute,status,active_session_id,generation,stopped_at_ms,created_at_ms,updated_at_ms FROM tasks WHERE id=?")
    ...
    async fn list_tasks(&self, limit: u32) -> Result<Vec<Task>, StorageError> {
        let rows = sqlx::query("SELECT id,agent_id,device_id,conversation_scope_hash,title,source,project_folder,allow_execute,status,active_session_id,generation,stopped_at_ms,created_at_ms,updated_at_ms FROM tasks WHERE NOT EXISTS (SELECT 1 FROM subagent_runs WHERE subagent_runs.child_task_id=tasks.id) ORDER BY updated_at_ms DESC,id LIMIT ?")
