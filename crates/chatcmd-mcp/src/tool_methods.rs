...
tool_methods!(
    ...
    (
        ...
        "Start an isolated headless Chrome, Edge, or Brave session controlled through Chrome DevTools Protocol without moving the user's physical pointer or taking keyboard focus. Optional browser=chrome|edge|brave, startUrl=http(s)|about:blank, width, and height. The browser uses a temporary profile and localhost-only DevTools endpoint. Returns sessionId."
    ),
    (
        computer_observe,
        SessionArgs,
    ...
    (
        computer_act,
        ComputerActArgs,
        "Execute 1-32 ordered actions in an owned isolated browser session, then return an updated screenshot by default. Required sessionId and actions. Supported action types: click, double_click, move, drag, scroll, keypress, type, navigate, wait, screenshot. Coordinates use the viewport pixels reported by computer_observe. Batch only steps whose next coordinates do not depend on an unseen result; isolate the final Send, Submit, purchase, or other external communication action in its own call so action-time policy applies. Full success reports completedActionCount. If a batch stops after input may have occurred, executionWarning reports the definitely completed prefix and retryAction=false; observe the browser before continuing and do not replay the batch. If actions succeed but screenshot verification fails, verificationWarning says to observe again without repeating actions."
    ),
    (
    ...
    (
        ...
        DesktopObserveArgs,
        "Observe one eligible Windows application window by opaque windowId. By default returns a Windows.Graphics.Capture screenshot as native MCP image content plus structured UI Automation elements and an observationId. Optional includeScreenshot and includeElements can disable either channel. A screenshot returns screenshotToken; passing it as knownScreenshotToken on a later desktop observation or action omits an identical image and sets screenshotUnchanged=true. Capture does not move the pointer or take keyboard focus; minimized windows must be restored by the user. Login/authentication must be completed manually; authentication, password, and security surfaces remain hard-denied."
    ),
    (
        desktop_element_act,
        DesktopElementActArgs,
        "Perform one background UI Automation action against an element from a fresh desktop_window_observe snapshot without moving the pointer or taking focus. Required observationId, elementId, and action. Supported actions: invoke, set_value, toggle, select, expand, collapse. Optional observeAfter (default true) returns a fresh observation in the same result; includeScreenshot defaults true and includeElements defaults false. Pass knownScreenshotToken from a prior desktop screenshot to omit an identical image; screenshotUnchanged reports that match. Every attempt invalidates the input observation. If the action succeeds but its fused observation fails, completed remains true and verificationWarning says to observe again without repeating the action; never retry the action from that warning. This tool never silently falls back to physical input. The final action that sends email, messages, forms, or other communication as the user must be isolated in its own call and passes through the configured execution policy, including action-time approval when required, in any app. Login/authentication is manual; authentication dialogs, passwords, password managers, and security surfaces remain hard-denied even after approval. If a tool or platform refuses an operation, report its observed reason and stop that path; do not disguise or reroute the action to bypass the refusal."
    ),
    (
        desktop_input_begin,
    ...
    (
        desktop_input_act,
        DesktopInputActArgs,
        "Execute 1-32 ordered physical mouse/keyboard actions in an active desktop takeover. Required inputSessionId and actions. Supported types: click, double_click, move, drag, scroll, keypress, type, wait. Batch only steps whose next coordinates do not depend on an unseen result; any final Send, Submit, purchase, or other external communication action must be isolated in its own call. Optional observeAfter (default true) returns a fresh observation in the same result; includeScreenshot defaults true and includeElements defaults false. Pass knownScreenshotToken from a prior desktop screenshot to omit an identical image; screenshotUnchanged reports that match. Full success returns completedActionCount equal to actions.length. If execution is interrupted after input may have occurred, the result returns the definitely completed prefix when known plus executionWarning with retryAction=false; observe again and never replay the batch. If the actions succeed but fused observation fails, active remains true and verificationWarning says to observe again without repeating the actions; never retry input from that warning. Coordinates are relative to the target window. The session cancels instead of typing if ESC is pressed, the target closes, or another app receives focus. Final communication as the user passes through the configured execution policy, including action-time approval when required, in any app. Login/authentication is manual; approval never bypasses authentication, password, password-manager, or security hard denies. If a tool or platform refuses an operation, report its observed reason and stop that path; do not disguise or reroute the action to bypass the refusal."
    ),
    (
        desktop_input_end,
        DesktopInputEndArgs,
