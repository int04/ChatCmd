    ...
    fn server_instructions_require_regular_progress_updates() {
        ...
        assert!(
            SERVER_INSTRUCTIONS.contains(
                "build, test, lint, search, Git operation, command, deployment"
            )
        );
        assert!(SERVER_INSTRUCTIONS.contains("not private chain-of-thought"));
        assert!(SERVER_INSTRUCTIONS.contains("every tiny mechanical call"));
