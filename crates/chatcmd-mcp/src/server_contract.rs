...
mod tests {
    ...
    #[test]
    fn server_instructions_keep_no_sampling_work_in_parent_conversation() {
        assert!(SERVER_INSTRUCTIONS.contains("parentContinuation"));
        assert!(SERVER_INSTRUCTIONS.contains("browser child fallback was intentionally disabled"));
        assert!(
            SERVER_INSTRUCTIONS
                .contains("continue the delegated work directly in the parent conversation")
        );
        assert!(SERVER_INSTRUCTIONS.contains("do not open or create another ChatGPT conversation"));
