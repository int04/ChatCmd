...
mod tests {
    ...
    #[test]
    fn initialization_failure_does_not_require_an_impossible_finalizer() {
        assert!(SERVER_INSTRUCTIONS.contains("INITIALIZATION FAILURE RULE"));
        assert!(SERVER_INSTRUCTIONS.contains("accepted=true and userMessageSynced=true"));
        assert!(SERVER_INSTRUCTIONS.contains("do not call task tools or agent_turn_complete"));
        assert!(SERVER_INSTRUCTIONS.contains("unknown upstream cause must remain unknown"));
        let methods = include_str!("tool_methods.rs");
        assert!(methods.contains("do not call this tool for the unsynchronized turn"));
    }
