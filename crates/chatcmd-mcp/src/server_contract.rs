...
mod tests {
    ...
    fn server_instructions_require_lazy_tool_discovery_in_same_turn() {
        assert!(SERVER_INSTRUCTIONS.contains("TOOL DISCOVERY RULE"));
        assert!(SERVER_INSTRUCTIONS.contains("lazy-load only a subset of tool schemas"));
        assert!(SERVER_INSTRUCTIONS.contains("connector/tool discovery mechanism"));
        assert!(SERVER_INSTRUCTIONS.contains("in the same turn"));
    ...
    fn server_instructions_require_strong_progress_updates() {
        ...
        assert!(SERVER_INSTRUCTIONS.contains("before skills_list or any other substantive tool call"));
        assert!(SERVER_INSTRUCTIONS.contains("what the user asked for and what you are going to do next"));
        assert!(SERVER_INSTRUCTIONS.contains("PROGRESS CADENCE RULE"));
        assert!(SERVER_INSTRUCTIONS.contains("throughout the entire turn, not only near the beginning"));
        assert!(SERVER_INSTRUCTIONS.contains("never perform more than 2 substantive non-progress tool calls"));
        assert!(SERVER_INSTRUCTIONS.contains("POST-ACTION REFLECTION RULE"));
        assert!(SERVER_INSTRUCTIONS.contains("meaningful file read or code inspection"));
        ...
        assert!(SERVER_INSTRUCTIONS.contains("completed=false"));
        assert!(SERVER_INSTRUCTIONS.contains("waitTimedOut=true"));
        assert!(SERVER_INSTRUCTIONS.contains("Do not silently perform repeated pending shell polls"));
