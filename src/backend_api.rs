...
const RELEASE_BACKEND_API_URL: &str = "http://127.0.0.1:5121";
...
impl BackendApiClient {
    ...
    async fn request_once(
        ...
        let encrypted_body = if body.is_empty() {
            ...
            encrypt_packet(&session.cipher, body, aad.as_bytes())
                .ok_or_else(|| anyhow!("encrypt backend request"))?
        };
        ...
        if !encrypted_body.is_empty() {
            request = request.body(encrypted_body);
        }
        let response = request
            .send()
            .await
            .context("send encrypted backend request")?;
        ...
        let body = if bytes.is_empty() || status == StatusCode::NO_CONTENT {
            ...
            if !is_encrypted {
                bail!("backend API returned plaintext response");
            }
            let aad = api_aad(
                "response",
                method.as_str(),
        ...
        Ok(RequestAttempt::Complete(BackendApiResponse {
    ...
    async fn handshake(&self) -> Result<BackendSession> {
        let handshake_cipher =
            handshake_cipher().ok_or_else(|| anyhow!("build backend handshake cipher"))?;
        ...
        let response = self
            ...
            .send()
            .await
            .context("send backend crypto handshake")?;
        if !response.status().is_success() {
            bail!("backend crypto handshake failed ({})", response.status());
        }
        let response_packet = response
            .bytes()
            .await
            .context("read backend server hello")?;
        ...
        let server: ServerHello =
            serde_json::from_slice(&response_plaintext).context("parse backend server hello")?;
        if server.message_type != "crypto.serverHello"
        ...
        let server_public_bytes = URL_SAFE_NO_PAD
            ...
            .context("decode backend public key")?;
        let server_public =
            PublicKey::from_sec1_bytes(&server_public_bytes).context("parse backend public key")?;
        let shared = secret.diffie_hellman(&server_public);
        let salt = URL_SAFE_NO_PAD
            .decode(server.salt)
            .context("decode backend HKDF salt")?;
        let hkdf = Hkdf::<Sha256>::new(Some(&salt), shared.raw_secret_bytes().as_slice());
        let mut key = [0_u8; 32];
        hkdf.expand(HKDF_INFO, &mut key)
            .map_err(|_| anyhow!("derive backend session key"))?;
        let cipher = Aes256Gcm::new_from_slice(&key)
            .map_err(|_| anyhow!("create backend session cipher"))?;
        key.fill(0);
        Ok(BackendSession {
...
enum RequestAttempt {
    Complete(BackendApiResponse),
    Reset,
}

fn configured_backend_url() -> String {
    let value = if cfg!(debug_assertions) {
        DEBUG_BACKEND_API_URL
    } else {
        RELEASE_BACKEND_API_URL
    };
...
fn decrypt_packet(cipher: &Aes256Gcm, packet: &[u8], aad: &[u8]) -> Option<Vec<u8>> {
    if packet.len() <= 13 || packet[0] != PROTOCOL {
        return None;
    }
    cipher
        .decrypt(
...
mod tests {
    ...
    fn debug_default_is_local_backend() {
        if cfg!(debug_assertions) {
            assert_eq!(configured_backend_url(), DEBUG_BACKEND_API_URL);
        }
    }
    #[test]
    fn packet_round_trip_binds_aad() {
        ...
        assert_eq!(
            decrypt_packet(&cipher, &packet, aad.as_bytes()).expect("decrypt"),
            b"{\"hello\":1}"
        );
    ...
    async fn local_dotnet_backend_interop_when_enabled() {
        ...
        let client = BackendApiClient::new().expect("backend client");
        let response = client
