...
Setting the Sub-Agent count above `0` is structured opt-in: the model decides from the request's meaning and task shape whether delegation is useful. ChatCMD does not classify the user message or require keywords in any language. Set the count to `0` to disable new child Agents.

How it works:

1. **Delegate a focused job.** The parent creates or reuses a child through `agent_subagent_start`, providing a name, request, optional scope context such as files, effects, dependencies, acceptance criteria and project context, plus an optional bounded safe-read approval grant.
...
3. **Run with the same safety boundaries.** Delegation does not widen server policy. Scope-context fields such as `allowedFiles` and `allowedEffects` instruct the child but do not themselves grant authority or create a server-side sandbox. Normal tool authorization, workspace/path policy, execution approval, lifecycle, cancellation, and resource budgets still apply. An optional `approvalGrant` can inherit only a bounded portion of an already approved parent safe-read grant; Git, process, write, and agent-lifecycle operations continue through their normal approval path.
