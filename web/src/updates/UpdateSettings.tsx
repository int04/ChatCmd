import { AlertTriangle, BadgeCheck, CheckCircle2, Cpu, Download, ExternalLink, FileCheck2, FileText, Laptop, PackageOpen, RefreshCw, RotateCcw, ShieldCheck, Sparkles } from 'lucide-react';
  ...
  return <div className="update-settings update-settings-pro">
    <section className={`update-settings-hero ${hasUpdate ? 'has-update' : ''}`}>
      ...
      <div className="update-settings-hero-copy">
        <span className={`update-settings-status-pill ${hasUpdate ? 'available' : 'current'}`}><i />{hasUpdate ? copy.popupBadge : copy.upToDate}</span>
    ...
    <div className="update-settings-main-grid">
      <div className="update-settings-left">
        <section className="update-settings-version-panel">
          ...
          <div className="update-settings-device-row">
            ...
            <div>{status.checksumVerified ? <BadgeCheck /> : <ShieldCheck />}<span><small>{status.assetName || 'Updater'}</small><strong>{status.checksumVerified ? copy.checksumVerified : status.downloadAvailable || !hasUpdate ? copy.updaterReady : copy.updaterUnavailable}</strong></span></div>
          </div>
        </section>
      ...
      <section className="update-settings-note-panel">
        <header>
          <span><FileText /></span>
          <div><small>{copy.releaseNotes}</small><strong>{status.releaseName || (hasUpdate ? `ChatCMD ${latestVersion}` : copy.currentVersion)}</strong></div>
          {status.releaseUrl && <a className="icon-button" href={status.releaseUrl} target="_blank" rel="noreferrer" aria-label={copy.openRelease} title={copy.openRelease}><ExternalLink /></a>}
        </header>
...
function UpdateState({ status }: { status: UpdateStatus }) {
  ...
  if (status.phase === 'upToDate') return <StateMessage icon={<CheckCircle2 />} title={status.latestVersion ? copy.upToDate : copy.noPublishedVersion} body={status.message ?? undefined} />;
  if (status.updateAvailable && !status.downloadAvailable) return <StateMessage icon={<AlertTriangle />} title={copy.unavailable} body={status.message ?? undefined} />;
  if (status.phase === 'readyToRestart') return <StateMessage icon={<CheckCircle2 />} title={copy.ready} body={copy.readyInstruction} />;
  if (status.phase === 'restarting') return <StateMessage icon={<RotateCcw className="spin" />} title={copy.restarting} body={status.message ?? undefined} />;
  if (['downloading', 'verifying', 'extracting', 'preparing'].includes(status.phase)) return <UpdateProgress status={status} />;
  return null;
}

function UpdateProgress({ status }: { status: UpdateStatus }) {
  const copy = updateCopy();
  const title = progressTitle(status.phase, copy);
  const percentage = status.progressPercent ?? 0;
  return <section className="update-progress-card" aria-live="polite">
    ...
    <div className="update-steps">
      ...
      <UpdateStep label={copy.verifyStep} state={stepState(status.phase, 'verifying')} icon={<FileCheck2 />} />
...
function stepState(current: UpdatePhase, step: UpdatePhase): 'done' | 'active' | 'pending' {
  const order: UpdatePhase[] = ['downloading', 'verifying', 'extracting', 'preparing', 'readyToRestart'];
...
function progressTitle(phase: UpdatePhase, copy: ReturnType<typeof updateCopy>) {
  if (phase === 'downloading') return copy.downloading;
  if (phase === 'verifying') return copy.verifying;
  if (phase === 'extracting') return copy.extracting;
...
async function waitForRestart(targetVersion: string) {
  for (let attempt = 0; attempt < 180; attempt += 1) {
    ...
    try {
      ...
      // The local server is expected to be briefly unavailable while the installer swaps files.
    }
  }
  throw new Error('ChatCMD did not come back online after the update. Please start ChatCMD manually and review the update error log.');
}
