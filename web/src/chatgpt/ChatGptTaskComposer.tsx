...
export function ChatGptTaskComposer({ taskId }: { taskId: string }) {
  ...
  return <>
    ...
    {folderMenuOpen && createPortal(<Modal className="workspace-folder-modal" title={tr('Choose project folder')} description={tr('Choose a saved project or open the folder picker on this computer.')} close={() => !folderPicking && setFolderMenuOpen(false)}><div className="workspace-folder-choices"><div className="workspace-folder-project-list">{projects.loading ? <p className="workspace-folder-empty"><LoaderCircle className="spin" /> {tr('Loading projects…')}</p> : projects.data?.length ? projects.data.map((project) => <button className={`workspace-folder-project ${canonicalProjectPath(attachedProjectFolder) === canonicalProjectPath(project.path) ? 'selected' : ''}`} type="button" onClick={() => { setAttachedProjectFolder(project.path); setFolderMenuOpen(false); }} key={project.id}><strong>{project.name}</strong><small>{project.path}</small></button>) : <p className="workspace-folder-empty">{projects.error || tr('No saved projects yet.')}</p>}</div><button className="workspace-folder-browse" type="button" onClick={() => void pickFolder()} disabled={folderPicking}>{folderPicking ? <LoaderCircle className="spin" /> : <FolderOpen />}<span><strong>{tr('Choose folder')}</strong><small>{tr('Open the folder picker on this computer')}</small></span></button></div></Modal>, document.body)}
    {pluginMenuOpen && createPortal(<Modal className="workspace-folder-modal" title={tr('Choose plugin')} description={tr('The selected plugin applies only to the next message.')} close={() => setPluginMenuOpen(false)}><div className="workspace-folder-choices"><div className="workspace-folder-project-list">{agents.loading ? <p className="workspace-folder-empty"><LoaderCircle className="spin" /> {tr('Loading plugins…')}</p> : enabledAgents.length ? enabledAgents.map((agent) => <button className={`workspace-folder-project ${attachedAgentId === agent.id ? 'selected' : ''}`} type="button" onClick={() => { setAttachedAgentId(agent.id); setPluginMenuOpen(false); }} key={agent.id}><strong>@{agent.name}</strong><small>{tr('Plugin enabled')}</small></button>) : <p className="workspace-folder-empty">{agents.error || tr('No enabled plugins.')}</p>}</div></div></Modal>, document.body)}
  </>;
}
