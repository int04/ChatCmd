...
export interface ApprovalGrant { id: Id; allowedTools: string[]; pathScopes: { path: string; kind: 'exact' | 'subtree' }[]; maxCalls: number; usedCalls: number; maxFilesScanned?: number; usedFilesScanned: number; maxBytesRead?: number; usedBytesRead: number; expiresAtUtc: string; state: string }
