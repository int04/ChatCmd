          ...
          {activeTab === 'execution' && <div className="execution-settings">
            ...
            <div className="settings-section-block">
              ...
              <div className="settings-control-grid">
                ...
                <SettingField label={tr('Sub-agent count')} hint={tr('Maximum child Agents running globally at once.')} detail={tr('When the limit is reached, new child Agents wait for a free slot. As soon as one Agent finishes, the next waiting Agent can start immediately.')}><select value={value.subagentConcurrency} onChange={(event) => update('subagentConcurrency', Number(event.target.value))}>{[1, 2, 3, 4, 5].map((count) => <option key={count} value={count}>{count}</option>)}</select></SettingField>
              </div>
            </div>
          </div>}
