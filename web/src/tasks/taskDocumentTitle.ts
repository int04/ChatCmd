...
const DEFAULT_TITLE = 'ChatCMD · Local Control';
...
export function useTaskDocumentTitle(tasks: Task[] | undefined) {
  ...
  useEffect(() => {
    ...
    for (const task of tasks) {
      ...
      activities.current.set(task.id, { label: 'Đang suy nghĩ', timestamp: Date.parse(task.updatedAtUtc) || Date.now() });
      changed = true;
    }
...
export function titleLabelForEvent(event: TimelineEvent) {
  ...
  if (event.type === 'progress') return 'Đang suy nghĩ';
  if (event.type === 'status') {
    const status = stringValue(payload.status);
    return ['completed', 'failed', 'stopped', 'interrupted'].includes(status) ? null : 'Đang suy nghĩ';
  }
  if (event.type === 'tool_result') return 'Đang suy nghĩ';
  if (event.type !== 'tool_call') return null;
  const status = stringValue(payload.status);
  if (status && status !== 'started') return 'Đang suy nghĩ';
  const tool = stringValue(payload.tool) || 'tool';
  const target = compactTitleTarget(payload.input);
  if (/^(?:fs_read_text|fs_list|fs_stat|fs_directory_sizes|view_image|file_download|skill_read)$/i.test(tool)) return `Đang đọc${target ? ` ${target}` : ''}`;
  if (tool === 'fs_search' || tool === 'fs_find') return `Đang tìm${target ? ` ${target}` : ''}`;
  if (/^(?:apply_patch|fs_write_text|fs_replace_text|file_upload)$/i.test(tool)) return `Đang sửa${target ? ` ${target}` : ''}`;
  if (tool === 'fs_create_directory') return `Đang tạo${target ? ` ${target}` : ''}`;
  if (/^(?:file_delete|empty_directory_delete|workspace_temp_cleanup|fs_delete)$/i.test(tool)) return `Đang xóa${target ? ` ${target}` : ''}`;
  if (tool === 'fs_copy') return `Đang sao chép${target ? ` ${target}` : ''}`;
  if (tool === 'fs_move') return `Đang di chuyển${target ? ` ${target}` : ''}`;
  if (tool.startsWith('git_')) return `Đang thao tác Git${target ? ` · ${target}` : ''}`;
  if (isCommandInputTool(tool)) return 'Đang chạy lệnh';
  return `Đang dùng ${tool || 'công cụ'}`;
}
...
function updateTitleActivity(activities: Map<string, TitleActivity>, commandBuffers: Map<string, CommandTitleBuffer>, event: TimelineEvent) {
  ...
  if (event.type === 'status' && ['completed', 'failed', 'stopped', 'interrupted'].includes(eventStatus)) {
    const changed = activities.delete(taskId) || commandBuffers.delete(taskId);
    return changed;
  }
  if (event.type === 'terminal_output') {
    ...
    activities.set(taskId, { label: 'Đang suy nghĩ', timestamp: eventTimestamp(event) });
    return true;
  }
...
function isCommandInputTool(tool: string) {
  return tool === 'shell_write' || /terminal.*input|execute|command/i.test(tool);
}

function commandOutputFinished(text: string) {
  return !!text && (/__CMDGPT_DONE_[A-Za-z0-9]+__/i.test(text) || /(?:^|\r?\n)PS\s+[^>\r\n]+>\s*$/i.test(text));
}

function eventTimestamp(event: TimelineEvent) {
  return Date.parse(event.occurredAt) || Date.now();
}
