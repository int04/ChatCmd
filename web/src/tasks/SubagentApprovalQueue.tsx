...
export function SubagentApprovalQueue({ approvals, onResolved }: { approvals: SubagentApproval[]; onResolved: (activityId: string) => void }) {
  ...
  return <section className="subagent-approval-queue" aria-label={tr('Subagent permission requests')} aria-live="polite">
    ...
    <div className="subagent-approval-list">{approvals.map((approval) => <article className="subagent-approval-alert" key={`${approval.childTaskId}:${approval.activityId}`}>
      ...
      <ApprovalDecisionActions target={{ taskId: approval.childTaskId, activityId: approval.activityId, turnId: approval.childTurnId }} reusable={isSafeReadApproval(approval.input)} onResolved={() => onResolved(approval.activityId)} />
    </article>)}</div>
  </section>;
}
...
function isSafeReadApproval(input: unknown) { if (!input || typeof input !== 'object') return false; const risk = (input as { riskClass?: unknown }).riskClass; return risk === 'metadataRead' || risk === 'contentRead' || risk === 'computeRead'; }
