...
export function TaskCodeViewer({ code, path, language, startLine = 1, label }: TaskCodeViewerProps) {
  ...
  return <div className="task-code-viewer">
    ...
    <Highlight theme={themes.vsDark} code={normalized} language={resolved}>
      {({ className, style, tokens, getLineProps, getTokenProps }) => <pre className={`${className} task-code-scroll`} style={style} tabIndex={0} aria-label={label}>
        <code>{tokens.map((line, index) => <span {...getLineProps({ line })} className="task-code-line" key={index}>
          <span className="task-code-line-number" aria-hidden="true">{firstLine + index}</span>
