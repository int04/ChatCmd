...
export function StatusBadge({ state, label }: { state: string; label?: string }) {
  ...
  return <span className={`status-badge ${good ? 'good' : bad ? 'bad' : 'warn'}`}><i aria-hidden="true" />{label ?? state}</span>;
}
export function Loading({ label = 'Loading' }: { label?: string }) { return <div className="state-panel" role="status"><LoaderCircle className="spin" /><strong>{label}</strong></div>; }
...
export function ErrorState({ message, retry }: { message: string; retry?: () => void }) { return <div className="state-panel error-state" role="alert"><CircleX /><strong>Could not load data</strong><span>{message}</span>{retry && <button className="button secondary" onClick={retry}><RefreshCw />Retry</button>}</div>; }
export function ProblemBanner({ message, clear }: { message?: string; clear?: () => void }) { return message ? <div className="problem-banner" role="alert"><AlertTriangle /><span>{message}</span>{clear && <button className="icon-button" aria-label="Dismiss error" onClick={clear}><X /></button>}</div> : null; }
...
export function Modal({ title, description, children, close, dangerous = false }: { title: string; description?: string; children: React.ReactNode; close: () => void; dangerous?: boolean }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><div ref={panel} className={`modal ${dangerous ? 'dangerous' : ''}`} role={dangerous ? 'alertdialog' : 'dialog'} aria-modal="true" aria-labelledby={titleId}><header><div><h2 id={titleId}>{title}</h2>{description && <p>{description}</p>}</div><button className="icon-button" aria-label="Close dialog" onClick={close}><X /></button></header>{children}</div></div>;
}
export function formatTime(value?: string) { if (!value) return '—'; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(date); }
export function healthLabel(state: HealthState) { return state === 'unknown' ? 'Not reported' : state[0].toUpperCase() + state.slice(1); }
